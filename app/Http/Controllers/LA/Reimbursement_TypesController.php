...
class Reimbursement_TypesController extends Controller {
    ...
    public $listing_cols = ['id', 'name', 'verification_level', 'document_required', 'limit', 'limit_variance', 'status','hard_copy_accepted'];
    ...
    public function index() {
        ...
        $this->listing_cols = ['id', 'name', 'verification_level', 'document_required', 'limit', 'limit_variance', 'status','hard_copy_accepted'];
    ...
    public function dtajax() {
        $this->listing_cols = ['id', 'name',DB::raw("(case when (verification_level = 1) THEN 'Lead' ELSE 'Manager' end) as verification_level"),'document_required', 'limit', 'limit_variance', DB::raw("(case when (status = 0) THEN 'Inactive' ELSE 'Active' end) as status"),'hard_copy_accepted'];

        $values = DB::table('reimbursement_types')->select($this->listing_cols)->whereNull('deleted_at');
        $out = Datatables::of($values)->make();
        $data = $out->getData();
          $col_arr = ['id', 'name', 'verification_level', 'document_required', 'limit', 'limit_variance','status','hard_copy_accepted'];
