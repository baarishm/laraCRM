...
class Comp_Off_ManagementsController extends Controller
{
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Comp_Off_Managements", "create")) {
			...
			$insert_id = Module::insert("Comp_Off_Managements", $request);
	...
	public function update(Request $request, $id)
	{
		if(Module::hasAccess("Comp_Off_Managements", "edit")) {
			...
			$insert_id = Module::updateRow("Comp_Off_Managements", $request, $id);
	...
	{
		$values = DB::table('comp_off_managements')->select($this->listing_cols)->whereNull('deleted_at');
		...
		for($i=0; $i < count($data->data); $i++) {
			...
			if($this->show_action) {
				...
				if(Module::hasAccess("Comp_Off_Managements", "delete")) {
					$output .= Form::open(['route' => [config('laraadmin.adminRoute') . '.comp_off_managements.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline']);
