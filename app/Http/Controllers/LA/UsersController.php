...
class UsersController extends Controller {
    ...
    public function show($id) {
        if (Module::hasAccess("Users", "view") && ((Session::get('role')  == 'superAdmin') || (Session::get('role')  != 'superAdmin' && $id == Auth::user()->id))) {
        ...
        for ($i = 0; $i < count($data->data); $i++) {
            ...
            if ($this->show_action) {
                ...
                if (Module::hasAccess("Users", "delete")) {
                    $output .= Form::open(['route' => [config('laraadmin.adminRoute') . '.users.destroy', $data->data[$i][0]], 'method' => 'delete', 'style' => 'display:inline', 'class' => 'delete']);
